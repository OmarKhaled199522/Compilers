...
bool Scanner::check_RHS (string str)
{
    //
    // any , - | two side are operand
    return true;
}
...
vector<char> Scanner::range_chars (char a, char b)
{
    vector<char> v;
    for (int j = a ; j <= b ; j++)
    {
...
{
    nfa.print_graph(nfa.get_nfa_graph());
}


void Scanner::print_vector_of_string(vector<string> s)
{
